...
- Results below 80% match (70 where the slider blends both axes) wait behind a button at the end of the list, which opens them ten percentage points at a time and names the range before it does. There is no knob for it: a control for "how similar" on a site about similarity was bloat, and the banding is what keeps the sorts meaningful, since sorting one band by price asks "the cheapest card that matches this well" rather than "the cheapest card with any resemblance at all"
